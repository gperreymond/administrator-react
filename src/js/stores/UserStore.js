...
function fail(error) {
  ...
  if ( error.message===FluxUserConstants.ERROR_BAD_AUTHENTIFICATION) {
    _message = 'Email/Password incorrects'
  } else {
...
const UserStore = _.extend({}, EventEmitter.prototype, {
  ...
  // return message
  getMessage: function() {
    debug('getMessage() _message=%o', _message);
    return _message;
  },

  // return message
  getToken: function() {
    debug('getToken() _token=%o', _token);
    return _token;
  },
  ...
  getConnected: function() {
    debug('getConnected() _connected=%o', _connected);
    return _connected;
  },
...
AppDispatcher.register(function(payload) {
  ...
  switch(action.actionType) {
    ...
    // fail
    case FluxUserConstants.ERROR_INVALID_TOKEN:
      debug(action.data);
      browserHistory.push('/login');
      break;

    // login success
    case FluxUserConstants.USER_LOGIN_SUCCESS:
      loginSuccess(action.data);
      break;

    // fail
    case FluxUserConstants.USER_FAIL:
      fail(action.data);
      break;
